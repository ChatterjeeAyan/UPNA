...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=20) ## Default is 20
    parser.add_argument('--eval_steps', type=int, default=1)
    parser.add_argument('--runs', type=int, default=1) ## Was 10
    ...
    pos_train_nodes, pos_val_test_nodes = train_test_split(all_nodes, test_size=0.2, random_state=42876)  #71139  #53517  #83959 #73183 #42876
    pos_val_nodes, pos_test_nodes  = train_test_split(pos_val_test_nodes, test_size=0.5, random_state=88434) #55054 #39830 #63909 #60742 #88434
    ...
    protvec_metadata = np.load('dataset/protvec_embeddings.npy').astype(float)
    protvec_metadata_normalized = normalize(protvec_metadata, axis=1, norm='l2') #Normalize
    protvec_random = np.random.rand(protvec_metadata_normalized.shape[0],protvec_metadata_normalized.shape[1])

    x = torch.tensor(protvec_random).to(torch.float)

    #import pdb; pdb.set_trace()

    ###### Don't use the Node2vec embeddings as they create shortcutss #####
