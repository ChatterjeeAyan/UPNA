...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=100) ## Was 200
    ...
    # = torch.load('embedding.pt', map_location='cpu').to(device)

    mol2vec = np.load('mol2vec_embeddings.npy').astype(float)
    mol2vec_normalized = normalize(mol2vec, axis=1, norm='l2') #Normalize
    mol2vec_normalized = crazyshuffle(mol2vec_normalized)    
    x = torch.tensor(mol2vec_normalized).to(torch.float).to(device)
